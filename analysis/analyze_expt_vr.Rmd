...
```{r load_data}
d_full <- read_csv(here("data", "expt_vr.csv"), show_col_types = FALSE) %>% 
  group_by(name) %>% 
...
all_results_phantom <- d_trials %>% 
  ...
  geom_hline(yintercept=67, color = "gray") +
  ...
  xlab("condition") + ylab("critical separation")
...
The y-axis here uses a logarithmic scale, and the grey horizontal lines show distances of 67 and 100. 

The plots here show estimates of the critical point -- ie the minimum distance at which the phantom illusion emerges. Using the SYNS prior, a uniform likelihood, and posterior mean as the estimator, our Bayesian model predicts that this critical point appears at a separation of 67 m between the observer and the lighthouse. At the time of developing and running the experiment, we did not have access to the SYNS prior, and generated predictions using a synthetic prior. Based on this synthetic prior we estimated that the critical point would like at a separation of 75 m, and preregistered the prediction that  ``the phantom lighthouse illusion will begin to emerge when the distance between viewer and lighthouse is between 50 and 100 m.''

For the staircase (red) and adjustment (cyan) methods, the median critical points obtained were  `r phantom_sc_med` and   `r phantom_adj_med` respectively.  These results seem broadly compatible with our prediction. The median for the adjustment method is outside the preregistered range, but not by much, and it's notable that the two methods provide converging evidence that the critical point is not far from the predicted value of 67 m. The results therefore suggest that (1) the phantom illusion is experienced in VR, that (2) as predicted, the illusion begins to emerge at a critical distance, and (3) this critical distance is around 67-100 m. 
...
Here we've plotted the 95% credibility interval on the median of the posterior distribution. For the staircase data, the 95% credibility interval includes 67, the predicted median. For the adjustment data, the density is to the left of 67 if we don't filter the responses less than 1, and to the right of 67 if we do filter these responses. Either way, though, the 95% credibility interval includes 67
